...
#+cindex:IP
#+cindex:RFC 971
The /Internet Protocol/  (IP), outlined in [[https://www.ietf.org/rfc/rfc971.txt][RFC 971]], describes  the standard for
...
sender  would probably  like to  ensure  taht only  the receiver  can read  the
packet, rather than the packet being readableby any router along the way.
...
an ICMP timeout packet, as described in RFC 793, with its own address.  The
routers that cnnot or will not do so are represented with =* * *=.
...
the connection,  it responds with  a SYN  and acknowledge packet.  Finally, the
client  acknowledges  the  acknowledgment  and  both sides  have  agreed  on  a
connection.
...
#+cindex:TCP
#+cindex:Transport Control Protocol
...
know them). This  is the /Transport Control Protocol/ (TCP);  RFC 793 describes
